...
const MentalReportPage = ({ onBack, onAddRecord, navigate, user, student }) => {
    ...
    const performDeleteRecord = async (record) => {
        ...
        // Best-effort: delete AI report first to avoid orphans.
        if (hasAIReport) {
    ...
    const confirmCreateNewRecord = async () => {
        ...
        try {
            // 使用默认标题，不再使用日期
            const defaultTitle = t('mentalAssessment');
            const assessmentId = await createAssessment(studentId, 'mental', user, defaultTitle, backendLang);
            ...
            if (assessmentId) {
                ...
                if (navigate) {
                    navigate('/add-record/mental/data', {
                        state: {
                            student,
                            assessmentData: {
    ...
    return (
        <div className="min-h-screen text-white p-4 sm:p-6 pb-56 relative overflow-hidden bg-transparent">
            ...
            <div className="relative z-10 mb-8 sm:mb-10 flex items-center gap-3">
                ...
                <h1 className="title-workbench flex-1 min-w-0">{t('mentalHistory')}</h1>
                <button
...
export default MentalReportPage;
