/**
 * MentalPlan - 心理测评计划组件
 * 功能：用于制定心理训练计划，支持多个计划项，每个计划项包含标题和内容
 * 特性：
 *   - 支持预设标题选择（专注能力提升、心理韧性提升、自信与动机提升）或自定义标题
 *   - 支持动态添加/删除计划项
 *   - 支持语音输入和文本编辑
 *   - 卡片式布局，支持悬停效果
 * 使用场景：新增测评记录页面的心理测评-计划步骤
 */
...
const titleToTranslationKey = {
    "专注能力提升": "focusAbilityImprovement",
    "心理韧性提升": "mentalResilienceImprovement",
    "自信与动机提升": "confidenceAndMotivationImprovement",
};

const MentalPlanItem = React.forwardRef(({ item, updateItem, removeItem, showTitleSelector, setShowTitleSelector, setListeningId, listeningId, startListening }, ref) => {
    const { t } = useLanguage();
...
const MentalPlan = ({ data, update }) => {
    ...
    // 初始化数据结构，如果不存在
    useEffect(() => {
        if (data.mentalPlan === null || data.mentalPlan === undefined) {
            const newItem = {
            ...
            update('mentalPlan', [newItem], true);
            return;
        }

        if (Array.isArray(data.mentalPlan) && data.mentalPlan.length === 0) {
            const newItem = {
                id: crypto?.randomUUID?.() || Date.now().toString(),
                title: presetTitles[0],
                content: '',
        ...
        <div className="page-container px-2">
            <div className="page-header">
                <div className="page-title-group">
                    <h2 className="title-main">{t('mentalPlan')}</h2>
                    <p className="title-subtitle">{t('mentalPlanSubtitle')}</p>
                </div>
            </div>
